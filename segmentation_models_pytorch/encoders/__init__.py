...
def get_encoder(
        ...
        weights=None,
        output_stride=32,
        attention=None,  # not support the TimmUniversalEncoder
        **kwargs):
    if name.startswith("tu-"):
        name = name[3:]
        encoder = TimmUniversalEncoder(name=name,
    ...
    params.update(attention=attention)
    encoder = Encoder(**params)
    # ignore weights for pretrained encoders if attention is not None
    if weights is not None and attention is None:
        try:
            settings = encoders[name]["pretrained_settings"][weights]
        except KeyError as exc:
            raise KeyError(
                f'Wrong pretrained weights `{weights}` for encoder `{name}`. Available options are: {list(encoders[name]["pretrained_settings"].keys())}'
            ) from exc
...
def get_preprocessing_params(encoder_name, pretrained="imagenet"):
    ...
    if encoder_name.startswith("tu-"):
        ...
        if pretrained not in all_settings.keys():
            raise ValueError("Available pretrained options {}".format(
                all_settings.keys()))
