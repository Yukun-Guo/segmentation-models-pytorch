...
class PAB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pab_channels=64):
        ...
        self.bottom_conv = nn.Conv2d(in_channels,
                                     in_channels,
                                     kernel_size=3,
                                     padding=1)
        self.map_softmax = nn.Softmax(dim=1)
        self.out_conv = nn.Conv2d(in_channels,
                                  in_channels,
                                  kernel_size=3,
                                  padding=1)
    ...
    def forward(self, x):
        ...
        sp_map = self.map_softmax(sp_map.view(bsize,
                                              -1)).view(bsize, h * w, h * w)
...
class MAnetDecoder(nn.Module):
    ...
    def __init__(
        ...
        blocks = [
            ...
            if skip_ch > 0 else DecoderBlock(in_ch, skip_ch, out_ch, **kwargs)
