...
def get_sample(model_class):
    if model_class in [
            smp.Unet, smp.Linknet, smp.FPN, smp.PSPNet, smp.UnetPlusPlus,
            smp.MAnet
    ]:
...
@pytest.mark.parametrize(
    "model_class",
    [smp.FPN, smp.PSPNet, smp.Linknet, smp.Unet, smp.UnetPlusPlus])
def test_forward(model_class, encoder_name, encoder_depth, **kwargs):
    ...
    if model_class in [smp.UnetPlusPlus, smp.Linknet
        ...
        return  # skip mit_b*
    if model_class is smp.FPN and encoder_name.startswith(
            "mit_b") and encoder_depth != 5:
        return  # skip mit_b*
    ...
    model.eval()
    test_shape = encoder_depth == 5 and model_class != smp.PSPNet
    _test_forward(model, sample, test_shape)


@pytest.mark.parametrize("model_class", [
    smp.PAN, smp.FPN, smp.PSPNet, smp.Linknet, smp.Unet, smp.UnetPlusPlus,
    smp.MAnet, smp.DeepLabV3
])
...
def test_dilation(encoder_name):
    if (encoder_name in ["inceptionresnetv2", "xception", "inceptionv4"]
    ...
    assert shapes == [64, 32, 16, 8, 4,
                      4]  # last downsampling replaced with dilation
